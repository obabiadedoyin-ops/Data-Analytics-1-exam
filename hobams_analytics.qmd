...
Every dataset, every hypothesis, every result in this document maps to a decision I am actively

navigating as a founder. That is the spirit in which this study was conducted

**Data Collection & Sampling**

**Source & Collection Method**

The dataset was collected via a structured Google Forms survey designed and administered by the researcher in her capacity as Founder of Sweet Indulgence by Hobams. The survey was distributed through WhatsApp groups, Instagram direct messages and Instagram Stories — the same channels used to market the brand between May citation("corrplot") citation("ggcorrplot") citation("car") citation("ResourceSelection") citation("scales") citation("patchwork") citation("readr").
...
2\. \*\*Events per year\*\*: Google Forms misread the numeric input "2" as a date ("2026-02-03"). Resolved by detecting and recoding date-formatted entries.
...
#| label: eda-distributions-python
#| message: false
#| warning: false
...
# Pass R dataframe to Python
df_py = r.df.copy()

fig, axes = plt.subplots(2, 2, figsize=(12, 8))
fig.suptitle("EDA: Key Variable Distributions (Python/Seaborn)", 
             fontsize=14, fontweight="bold")

# Plot 1 - Income distribution
income_counts = df_py["income"].value_counts()
axes[0, 0].barh(income_counts.index, income_counts.values, 
                color=sns.color_palette("Blues_d", len(income_counts)))
axes[0, 0].set_title("Income Distribution")
axes[0, 0].set_xlabel("Count")

# Plot 2 - Area distribution
area_counts = df_py["area"].value_counts()
axes[0, 1].barh(area_counts.index, area_counts.values,
                color=sns.color_palette("Greens_d", len(area_counts)))
axes[0, 1].set_title("Respondents by Lagos Area")
axes[0, 1].set_xlabel("Count")

# Plot 3 - Events per year
events_counts = df_py["events_per_year"].value_counts()
axes[1, 0].barh(events_counts.index.astype(str), 
                events_counts.values,
                color=sns.color_palette("Oranges_d", len(events_counts)))
axes[1, 0].set_title("Events Hosted Per Year")
axes[1, 0].set_xlabel("Count")

# Plot 4 - Budget distribution
budget_data = df_py["budget_cleaned"].dropna()
axes[1, 1].hist(budget_data, bins=20, color="#2C7BB6", 
                edgecolor="white")
axes[1, 1].set_title("Distribution of Event Budget (₦)")
...
The confusion matrix shows how well the model classifies parents as premium or non-premium customers. Reading the results:

- **True Positives (TP):** Parents correctly predicted as premium — these are the customers Sweet Indulgence should prioritize in its outreach

- **True Negatives (TN):** Parents correctly predicted as non-premium

- **False Positives (FP):** Parents predicted as premium who are not — wasteful marketing spend

- **False Negatives (FN):** Premium parents the model missed — lost revenue opportunities

For Sweet Indulgence by Hobams, **minimising False Negatives is the priority** — a missed premium customer costs more than a wasted marketing contact. The model's sensitivity score confirms how well it captures the true premium segment.
